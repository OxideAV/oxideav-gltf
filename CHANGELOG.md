...
## [0.0.5](https://github.com/OxideAV/oxideav-gltf/compare/v0.0.4...v0.0.5) - 2026-09-01

### Other

- hide internal pub surface from rustdoc/semver (fleet rule 2026-09-01)

## [0.0.4](https://github.com/OxideAV/oxideav-gltf/compare/v0.0.3...v0.0.4) - 2026-08-30

### Other

- pin that in-between shapes have no glTF wire form
- weights channels: adopt the sampled-MorphWeights synthesis path + lossless read-back
- adopt the typed Mesh::target_names surface
- construct MorphTarget via a named-constructor helper — valid on both the published and master upstream surfaces
- construct MorphTarget through with_deltas — the type is non_exhaustive upstream
- cross-surface integration suite for the mesh3d 0.0.5 adoption
- adopt the typed Node::weights surface
- adopt the typed TextureRef::transform surface
- sampler fidelity: adopt the Option-shaped filter surface
- KHR_animation_pointer §Operation enclosure/presence rules
- full Object Model pointer registry — ObjectModel.md DOCS-GAP closed
- typed morph surface adoption + node.weights support per spec §5.25.9
...
- **Buffer `data:`-URI mediatype validation (spec §3.9.1)** — a new
  ...
  headers (`;charset=…;base64`) are handled by matching only the leading
  mediatype token; external (non-`data:`) URIs and the GLB-stored BIN
  buffer 0 (no `uri`) are untouched.
